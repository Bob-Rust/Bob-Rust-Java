...
public class DrawDialogNew extends JDialog {
	private static final Logger LOGGER = LogManager.getLogger(DrawDialogNew.class);
	...
	public DrawDialogNew(JDialog parent) {
		...
		JLabel lblClickInterval = new JLabel(RustUI.getString(RustUI.Type.SETTINGS_CLICKINTERVAL_LABEL));
		lblClickInterval.setToolTipText(RustUI.getString(RustUI.Type.SETTINGS_CLICKINTERVAL_TOOLTIP));
		...
		btnCalculateExactTime = new JButton(RustUI.getString(RustUI.Type.ACTION_CALCULATEEXACTTIME_BUTTON));
		...
		btnStartDrawing = new JButton(RustUI.getString(RustUI.Type.ACTION_STARTDRAWING_BUTTON));
		btnSelectColorPalette = new JButton(RustUI.getString(RustUI.Type.ACTION_SELECTCOLORPALETTE_BUTTON));
	...
	private void showPaletteWarning() {
		...
		JOptionPane.showMessageDialog(this, pane, RustUI.getString(RustUI.Type.ACTION_PALETTEWARNINGDIALOG_TITLE), JOptionPane.WARNING_MESSAGE);
	}
